...
export const generateLinkUnlock = async ({
  factoryAddress,
  chainId,
  claimHost,
  linkdropMasterAddress,
  ...
  version,
  campaignId,
  lock // Lock contract address
}) => {
  if (lock === null || lock === '') {
    throw new Error('Please provide lock address')
  }
  ...
  // Construct link
  const url = `${claimHost}/#/receive?weiAmount=${weiAmount}&tokenAddress=${tokenAddress}&tokenAmount=${tokenAmount}&expirationTime=${expirationTime}&version=${version}&chainId=${chainId}&linkKey=${linkKey}&linkdropMasterAddress=${linkdropMasterAddress}&linkdropSignerSignature=${linkdropSignerSignature}&campaignId=${campaignId}&lock=${lock}`
